...
static constexpr uint32_t COUNTDOWN_MS = 10'000;

static constexpr float FT2M = 0.3048f;
static constexpr float M2FT = 3.2808399f;
static constexpr float TARGET_ALT_FT = 3.0f;
static constexpr float TARGET_ALT_M  = TARGET_ALT_FT * FT2M;

static constexpr uint16_t TELEMETRY_PERIOD_MS = 50;
...
void FlightComputer::update() {
  ...
  const uint32_t since = now - phaseStartMs;
  ...
  switch (fcPhase) {
    ...
    case FlightPhase::COUNTDOWN: {
      setLED(true, true);
      int32_t sec_left = (COUNTDOWN_MS > since) ? int32_t((COUNTDOWN_MS - since)/1000) : 0;
  ...
  if ((fcPhase == FlightPhase::ASCENT ||
       ...
       fcPhase == FlightPhase::DESCENT) && !killLatched)
  {
...
bool FlightComputer::initSD() {
  ...
  int idx = 0;
  do {
    snprintf(name, sizeof(name), "flight_%03d.csv", idx++);
  } while (SD.exists(name) && idx < 1000);
...
void FlightComputer::buildPlantState(const IMURaw& r, float alt_m, float dt) {
...
void FlightComputer::runController() {
  ...
  pwmEsc = constrain(pwmEsc, ESC_MIN_US, ESC_MAX_US);
