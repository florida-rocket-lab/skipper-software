...
public:
    ...
    virtual void send(std::unique_ptr<BaseSerializable> data) = 0;
    virtual std::unique_ptr<BaseSerializable> receive(size_t buffer_size) = 0;  // Every communication protocol needs to receive and **deserialize** the data.
...
public:
    ...
    void send(std::unique_ptr<BaseSerializable> data) override;
...
void RadioCommunication::send(const std::unique_ptr<BaseSerializable> data)
{
    this->send(data.get());
}
void UARTCommunication::send(std::unique_ptr<BaseSerializable> data)
{
    this->send(data.get());
}
...
template <typename T>
std::unique_ptr<T> RadioCommunication::receive()
{
...
template <typename T>
std::unique_ptr<T> UARTCommunication::receive()
{
    std::unique_ptr<char[]> data_buffer = std::make_unique<char[]>(T::BUFFER_SIZE);
