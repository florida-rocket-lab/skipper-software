...
class EKF // Extended Kalman Filter
{
  public:
    EKF() {};
    struct SkipperState filter_data(const struct IMUData &imu_data);
  private:
...
{
  Teensy2Ground(): imu_data{}, skipper_state{}, control_signal{} {};
  Teensy2Ground(const struct IMUData &id, const struct SkipperState &ss, const struct ControlSignal &cs): imu_data(id), skipper_state(ss), control_signal(cs) {};
  const struct IMUData imu_data;
  const struct SkipperState skipper_state;
  const struct ControlSignal control_signal; // We're just forwarding our data to the ground station.
};
