...
struct IMUData get_imu_data()
{
  // TODO: retrieve IMU Data from a library/Drehmflight?
  return {};
}

struct SkipperState EKF::filter_data(const struct IMUData& imu_data)
{
  // TODO: filter an IMU Data struct using an extended Kalman filter.
  return {};
}

struct ControlSignal run_GNC(const struct SkipperState& state, const SkipperFlag flags=SkipperFlag::Idle)
{
...
void update_controls(const struct ControlSignal& control_signal)
{
...
struct Ground2Teensy read_message_from_nano()
{
...
void update_skipper_flags(const struct Ground2Teensy &command, enum class SkipperFlag &skipper_flag)
{
...
void send_message_to_nano(const struct IMUData &id, const struct SkipperState &ss, const struct ControlSignal &cs)
{
  struct Teensy2Ground t2g {id, ss, cs};
