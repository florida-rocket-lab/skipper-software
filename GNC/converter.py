...
if __name__ == '__main__':
    ...
    x_variables = ['u', 'v', 'w', 'phi', 'theta', 'psi', 'p', 'q', 'r']
    # Copied and pasted from the output of "eomAlgebra2.m" file.
    # Keep in mind though that the equations need to be in the order
    # of the x_variables.
    system = [
        "w0*(sin(phi0)*sin(psi0) + cos(phi0)*cos(psi0)*sin(theta0)) - v0*(cos(phi0)*sin(psi0) - cos(psi0)*sin(phi0)*sin(theta0)) - (cos(phi0)*sin(psi0) - cos(psi0)*sin(phi0)*sin(theta0))*(v - v0) + (sin(phi0)*sin(psi0) + cos(phi0)*cos(psi0)*sin(theta0))*(w - w0) + (psi0 - psi)*(v0*(cos(phi0)*cos(psi0) + sin(phi0)*sin(psi0)*sin(theta0)) - w0*(cos(psi0)*sin(phi0) - cos(phi0)*sin(psi0)*sin(theta0)) + u0*cos(theta0)*sin(psi0)) + (theta - theta0)*(w0*cos(phi0)*cos(psi0)*cos(theta0) - u0*cos(psi0)*sin(theta0) + v0*cos(psi0)*cos(theta0)*sin(phi0)) + (phi - phi0)*(v0*(sin(phi0)*sin(psi0) + cos(phi0)*cos(psi0)*sin(theta0)) + w0*(cos(phi0)*sin(psi0) - cos(psi0)*sin(phi0)*sin(theta0))) + u0*cos(psi0)*cos(theta0) + cos(psi0)*cos(theta0)*(u - u0)",
        "v0*(cos(phi0)*cos(psi0) + sin(phi0)*sin(psi0)*sin(theta0)) - w0*(cos(psi0)*sin(phi0) - cos(phi0)*sin(psi0)*sin(theta0)) - (psi0 - psi)*(w0*(sin(phi0)*sin(psi0) + cos(phi0)*cos(psi0)*sin(theta0)) - v0*(cos(phi0)*sin(psi0) - cos(psi0)*sin(phi0)*sin(theta0)) + u0*cos(psi0)*cos(theta0)) + (cos(phi0)*cos(psi0) + sin(phi0)*sin(psi0)*sin(theta0))*(v - v0) - (cos(psi0)*sin(phi0) - cos(phi0)*sin(psi0)*sin(theta0))*(w - w0) + (theta - theta0)*(w0*cos(phi0)*cos(theta0)*sin(psi0) - u0*sin(psi0)*sin(theta0) + v0*cos(theta0)*sin(phi0)*sin(psi0)) - (phi - phi0)*(v0*(cos(psi0)*sin(phi0) - cos(phi0)*sin(psi0)*sin(theta0)) + w0*(cos(phi0)*cos(psi0) + sin(phi0)*sin(psi0)*sin(theta0))) + u0*cos(theta0)*sin(psi0) + cos(theta0)*sin(psi0)*(u - u0)",
        "(v0*cos(phi0)*cos(theta0) - w0*cos(theta0)*sin(phi0))*(phi - phi0) - (theta - theta0)*(u0*cos(theta0) + w0*cos(phi0)*sin(theta0) + v0*sin(phi0)*sin(theta0)) - u0*sin(theta0) - sin(theta0)*(u - u0) + w0*cos(phi0)*cos(theta0) + v0*cos(theta0)*sin(phi0) + cos(phi0)*cos(theta0)*(w - w0) + cos(theta0)*sin(phi0)*(v - v0)",
